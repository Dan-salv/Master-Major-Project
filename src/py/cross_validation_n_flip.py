...
def nested_crossvalidation(model,param,X , Y):
    ...
    mean = {k:v.mean() for k, v in scores.items()}
...
def print_scores(scores):
    ...
    with open("data/training_data/scores/training_scores_n_flip.json", "w") as outfile:
...
def run_script():
    ...
    model3 = Pipeline(steps= [("om", over) , ("um", under) , ('kbest', kbest) ,('xgb', XGB)] )
    model4 = Pipeline(steps= [("om", over) , ("um", under ),("preprocess", data_transform),('kbest', kbest) ,('svm', SVM)] )
    

    params1, params2, params3, params4 = get_params()
    
    #svm_scores = nested_crossvalidation(model4,params4, X_train, y_train)
    knn_scores = nested_crossvalidation(model1,params1, X_train, y_train)
    xgb_scores = nested_crossvalidation(model3,params3, X_train, y_train)
