...
def split_data(data):
    ...
    label['encoded_label_binary'] = label["encoded_label"].apply(lambda x: 0 if x < 3 else 1)
    
    X = df
    y = label['encoded_label_binary'].array
...
def print_scores(scores):
    ...
    with open("data/training_data/scores/training_scores_binary_all.json", "w") as outfile:
...
def run_script():
    ...
    model1 = Pipeline(steps= [('kbest', kbest) ,('knn', KNN)] )
    model2 = Pipeline(steps= [('kbest', kbest) ,('rf', RFC)] )
    #model3 = Pipeline(steps= [('kbest', kbest) ,('svm', SVM)] )
    model4 = Pipeline(steps= [('kbest', kbest) ,('xgb', XGB)] )
