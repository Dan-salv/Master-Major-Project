...
def get_final_model(X_train,y_train):
    ...
    clf3 = XGBClassifier(num_class = 4)
    clf4 = SVC(probability=True, class_weight= "balanced")
    
    pipe = Pipeline(steps= [("om", over) , ("um", under),('kbest', kbest) ,('classifier', clf1)] )
    
    os_strategy = [
    {0:1200, 1:1200}, {0:1400, 1:1400}, {0:1800, 1:1800}]
...
def run_script():
    ...
    with open('provisional_multiclass.pkl', 'wb') as files:
